...
function longestSentence(text) {
  ...
  let compareText = sentences[0];

  for (let idx = 1; idx < sentences.length; idx++) {
    let currentLength = sentences[idx].split(/\s/).length;
    let compareTextLength = compareText.split(/\s/).length;

    if (compareTextLength < currentLength) {
      compareText = sentences[idx];
    }
  }
  console.log(`${compareText}\n\nThe longest sentence has ${compareText.split(/\s/).length} words.\n`);
}
...
//  they gave the last full measure of devotion -- that we here highly resolve that these dead shall not have died in vain -- that this nation, under God, shall have a
